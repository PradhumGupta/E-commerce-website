...
import { CheckCircleIcon, ShoppingBagIcon, HomeIcon, CalendarIcon, DollarSignIcon, ReceiptTextIcon } from 'lucide-react';
...
function PurchaseSuccessPage() {
  ...
  const handleSuccessPage = async () => {
    ...
    try {
        ...
        clearCart();
        setOrderDetails({
          orderId, // Use part of session_id as dummy order ID
          paymentStatus: 'Paid',
          orderDate: new Date().toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' }),
          totalAmount: (Math.random() * 500 + 50).toFixed(2), // Random amount for demo
          itemsCount: Math.floor(Math.random() * 5) + 1, // Random item count
        });
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-base-200 p-4">
        <div className="text-center text-base-content/70 text-xl">
          ...
          <br />
          <Link to="/" className="link link-hover text-primary mt-4 inline-block">
            Go to Homepage
          </Link>
        </div>
      </div>
    ...
    >
      ...
      >
        <div className="card-body p-0">
          ...
          <p className="text-base-content/80 text-lg mb-6">Thank you for your order.</p>
          ...
          {!isProcessing ? (
            <div className="text-left space-y-3 text-base-content/90 mb-8">
              ...
              <div className="flex items-center gap-3">
                ...
                <span className="text-accent font-bold">${orderDetails.totalAmount}</span>
              </div>
              <div className="flex items-center gap-3">
              ...
              <div className="flex items-center gap-3">
                ...
                <span className="text-success">{orderDetails.paymentStatus}</span>
              </div>
            </div>
          ) : (
          ...
          <div className="flex flex-col gap-3">
            <Link to="/" className="btn btn-primary btn-lg shadow-md hover:shadow-lg transition-all duration-200">
              <HomeIcon className="size-5" /> Continue Shopping
            </Link>
            <Link to="/my-orders" className="btn btn-outline btn-secondary btn-lg shadow-md hover:shadow-lg transition-all duration-200">
